...
func TestImageLoad(t *testing.T) {
	...
	for _, s := range errorTests {
		t.Run(s.path, func(t *testing.T) {
			_, err := GifFromImagePath(filepath.Join("testdata", s.path))
			if err.Error() != s.err {
	...
	t.Run("good.gif", func(t *testing.T) {
		_, err := GifFromImagePath(filepath.Join("testdata", "good.gif"))
		if err != nil {
...
func TestCurrentFrame(t *testing.T) {
	t.Run("good.gif", func(t *testing.T) {
		g, err := GifFromImagePath(filepath.Join("testdata", "good.gif"))
		if err != nil {
			t.Errorf("Expected: nil, Got: %v", err)
		}
