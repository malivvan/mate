...
func TestWindowManagerFocus(t *testing.T) {
	...
	wndB := window.NewWindow().SetRoot(rootB)
...
func TestWindowManagerSetZ(t *testing.T) {
	...
	wm.SetZ(windows[0], window.WindowZTop)
	...
	wm.SetZ(windows[5], window.WindowZBottom)
...
var minW = window.MinWindowWidth
var minH = window.MinWindowHeight
...
func TestWindowManagerDraw(t *testing.T) {
	...
	for _, wt := range WMDrawTests {
		wnd := window.NewWindow() // create a new window. Windows are not visible by default.
	...
	var testWindow window.Window = wm.NewWindow()
	...
	for i, wnd := range windows {
		rect := window.NewRect(wnd.GetRect())
...
func TestWindowManagerKeyboard(t *testing.T) {
	...
	wm.SetZ(wnd3, window.WindowZTop)
	inputHandler(testEventKey, testSetFocus)
	if lastPrimitive != wnd3.GetRoot() {
		t.Fatal("Expected last keypress to have gone to window3's root")
	}
...
func TestMaximizeRestore(t *testing.T) {
	...
	initialRect := window.NewRect(1, 2, 5, 9)
	...
	maximizedRect := window.NewRect(wndA.GetRect())
	wmSize := window.NewRect(wm.GetInnerRect())
	...
	restoredRect := window.NewRect(wndA.GetRect())
...
func TestModal(t *testing.T) {
	...
	w4 := wm.Window(4)
	w4.(*window.WindowBase).SetModal(true)
	setFocus(w4)
	wm.Draw(screen)
