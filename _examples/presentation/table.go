...
const tableBasic = `[green]func[white] [yellow]main[white]() {
    table := view.[yellow]NewTable[white]().
...
func Table(nextSlide func()) (title string, info string, content view.Primitive) {
	table := view.NewTable()
	table.SetFixed(1, 1)
	for row, line := range strings.Split(tableData, "\n") {
		for column, cell := range strings.Split(line, "|") {
			color := view.Styles.PrimaryTextColor
			if row == 0 {
				color = view.Styles.SecondaryTextColor
			} else if column == 0 {
				color = view.Styles.TertiaryTextColor
			}
			align := view.AlignLeft
			if row == 0 {
				align = view.AlignCenter
			...
			tableCell := view.NewTableCell(cell)
	...
	separator := func() {
		table.SetBorders(false)
		table.SetSelectable(false, false)
		table.SetSeparator(view.Borders.Vertical)
		code.Clear()
	...
	for _, tableText := range demoTableText {
		item := view.NewListItem(tableText.text)
