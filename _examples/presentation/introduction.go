...
// Introduction returns a view.List with the highlights of the view package.
func Introduction(nextSlide func()) (title string, info string, content view.Primitive) {
	list := view.NewList()
	...
	reset := func() {
		...
		for i, itemText := range listText {
			item := view.NewListItem(itemText[0])
