...
[green]import[white] [red]"code.rocketnine.space/tslocum/view"[white]

[green]func[white] [yellow]main[white]() {
	$$$

	root := view.[yellow]NewTreeNode[white]([red]"Root"[white]).
		[yellow]AddChild[white](view.[yellow]NewTreeNode[white]([red]"First child"[white]).
			[yellow]AddChild[white](view.[yellow]NewTreeNode[white]([red]"Grandchild A"[white])).
			[yellow]AddChild[white](view.[yellow]NewTreeNode[white]([red]"Grandchild B"[white]))).
		[yellow]AddChild[white](view.[yellow]NewTreeNode[white]([red]"Second child"[white]).
			[yellow]AddChild[white](view.[yellow]NewTreeNode[white]([red]"Grandchild C"[white])).
			[yellow]AddChild[white](view.[yellow]NewTreeNode[white]([red]"Grandchild D"[white]))).
		[yellow]AddChild[white](view.[yellow]NewTreeNode[white]([red]"Third child"[white]))
...
const treeBasicCode = `tree := view.[yellow]NewTreeView[white]()`

const treeTopLevelCode = `tree := view.[yellow]NewTreeView[white]().
		[yellow]SetTopLevel[white]([red]1[white])`

const treeAlignCode = `tree := view.[yellow]NewTreeView[white]().
		[yellow]SetAlign[white](true)`

const treePrefixCode = `tree := view.[yellow]NewTreeView[white]().
...
var (
	tree          = view.NewTreeView()
	treeNextSlide func()
	treeCode      = view.NewTextView()
)
...
func TreeView(nextSlide func()) (title string, info string, content view.Primitive) {
	treeNextSlide = nextSlide
	tree.SetBorder(true)
	tree.SetTitle("TreeView")

	// Add nodes.
	var add func(target *node) *view.TreeNode
	add = func(target *node) *view.TreeNode {
		node := view.NewTreeNode(target.text)
	...
	tree.SetSelectedFunc(func(n *view.TreeNode) {
