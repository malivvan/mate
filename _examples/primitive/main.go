...
func (r *RadioButtons) Draw(screen tcell.Screen) {
	...
	for index, option := range r.options {
		...
		view.Print(screen, []byte(line), x, y+index, width, view.AlignLeft, tcell.ColorYellow.TrueColor())
	}
}

// InputHandler returns the handler for this primitive.
func (r *RadioButtons) InputHandler() func(event *tcell.EventKey, setFocus func(p view.Primitive)) {
	return r.WrapInputHandler(func(event *tcell.EventKey, setFocus func(p view.Primitive)) {
		switch event.Key() {
		case tcell.KeyUp:
			r.currentOption--
